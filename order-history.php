...
if (isset($asID)) {
  ...
  // varmistetaan, että htun-tieto on annettu ja jos on, niin lähdetään hakemaan tietoa tietokannasta
      ...
      try {
        ...
        $sql = "SELECT * FROM Tuote as t
        INNER JOIN TilausRivi as tr
        ON tr.tuoteid = t.tuoteid
        INNER JOIN Tilaus as til
        ON til.tilausid = tr.tilausid
        WHERE til.asID = :asID";
        ...
        // jos $data on tyhjä, niin haettua asiakasta ei löytynyt
          $return['tilausID']  = "Asiakkuudella ei ole tilauksia";
